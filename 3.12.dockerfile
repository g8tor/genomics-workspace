FROM  python:3.12.9-slim-bullseye AS installer
...
RUN groupdel -f  dialout  && \
    ...
    apt-get --no-install-recommends -y install init direnv nginx gcc cssmin nano libz-dev libjpeg-dev libpcre3 libpcre3-dev  && \
    mkdir -p production media .venv/bin run logs src/static && \
