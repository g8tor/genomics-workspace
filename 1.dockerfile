...
ARG GID=1001 UID=1001 BASEDIR=/app/i5k APP_HOME=/app/i5k APP_USER=i5k
...
RUN groupdel -f  dialout  && \
    ...
    apt-get --no-install-recommends -y install direnv nginx gcc cssmin nano && \
...
ENTRYPOINT ["/app/i5k/.venv/bin/entry-point.sh"]