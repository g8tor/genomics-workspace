...
RUN groupdel -f  dialout  && \
    ...
    apt-get --no-install-recommends -y install init python3-psycopg2 sudo direnv nginx gcc cssmin nano libz-dev libjpeg-dev libpcre3 libpcre3-dev postgresql-client && \
    mkdir -p production media .venv/bin run logs static .local && \
    ln -s `pwd`/static production/static && \
    ln -s `pwd`/src/* production/ && \
    rm production/i5k && \
    cp -R src/i5k production/ && \
    chown -R  ${APP_USER}:${APP_USER} .local/ .venv/ && \
    ...
    sudo su -l ${APP_USER} -c "cd ${APP_HOME} && direnv allow ."  && \
    sudo su -l ${APP_USER} -c "cd ${APP_HOME}/production && direnv allow . " && \
    sudo su -l ${APP_USER} -c "cd ${APP_HOME}/training && direnv allow ."  && \
    chown  -R ${APP_USER}:${APP_USER} production  training && \
    chown ${APP_USER} /app
